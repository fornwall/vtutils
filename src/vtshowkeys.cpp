...
int main(int argc, char** argv)
{
	...
	char* send = getenv("send");
	if (send != NULL) { printf("%s", send); fflush(stdout); }

	if (set_cursor_app) decPrivateMode(1, true);
	if (set_keypad_app) decPrivateMode(66, true);
	if (mouse_mode > 0) { decPrivateMode(1000, true); decPrivateMode(mouse_mode, true); }
	...
	if (mouse_mode > 0) { decPrivateMode(1000, false); decPrivateMode(mouse_mode, false); }
